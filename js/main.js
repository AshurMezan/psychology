{// Скрипт для слайдера в блоке section_two
    document.addEventListener('DOMContentLoaded', function () {
        try {
            ...
            const fistArray = [
                "Психотерапевты сами ненормальные, неуравновешенные.",
                "Психотерапевты опасны, они постоянно залазят людям в мозги.",
                "Психотерапевты много говорят и ничего не делают. На эту работу способен любой человек.",
                "От психотерапии становится только хуже: людям становится больно, они плачут и начинают думать о смерти и сходят с ума.",
                "Психиатрическая больница переполнена людьми, которые были у психотерапевта.",
                "Психотерапевты — жулики.",
                "Психотерапевт думает только о том, чтобы урвать у вас как можно больше денег. Поэтому он специально сделает вас больным и зависимым!",
                "Человек должен сам решать свои проблемы. К психотерапевту обращаются только слабаки и безвольные.",
                "Психотерапевт после работы рассказывает о нас своим знакомым и смеется над нами!",
                "Раз никто не смог помочь, ваше страдание безнадежно, и какой-то там психотерапевт тем более не поможет.",
                "Вы не готовы к психотерапии. Это отнимает слишком много времени, которое лучше провести с друзьями за бокалом вина или чего-нибудь покрепче. Друзья вас поймут гораздо лучше и обязательно помогут.",
                "Зачем идти к психотерапевту, когда есть замечательные таблетки и что-то посерьезнее.",
                "Гадалки, колдуны и экстрасенсы помогут гораздо лучше и дешевле."
            ];
    
            let currentTextIndex = 0;
    
            if (!sliderText) {
                throw new Error('Элемент с классом "sclaider_text" не найден.');
            }
            if (!buttonLeft) {
                throw new Error('Элемент с классом "button_left" не найден.');
            }
            if (!buttonRight) {
                throw new Error('Элемент с классом "button_right" не найден.');
            }
...
{// C помощью javascript  решаем проблемы, которые упустили в css
    
        // Это кнопки для слайдера
        document.addEventListener('DOMContentLoaded', function () {
            try {
                ...
                if (btnLeftForSliderInSectionFour) {
                    ...
                    throw new Error('Элемент с id "btn_left_for_slider" не найден.');
                }
        
                if (btnRinghtForSliderInSectionFour) {
                    btnRinghtForSliderInSectionFour.style.top = '210px';
                } else {
                    throw new Error('Элемент с id "btn_right_for_slider" не найден.');
                }
            } catch (error) {
                console.error('Произошла ошибка:', error.message);
            }
        ...
        const nav_container = document.querySelector('.nav_conteiner');
        // Убедитесь, что элемент найден
        if (nav_container) {
            nav_container.style.paddingLeft = '15px';
        } else {
            console.error('Элемент с классом "nav_conteiner" не найден.');
        }
